...
class ClinVarVariant(MetaPubObject):
    ...
    Attributes:
        ...
        clinical_significance (ClinicalSignificance):
            ...
            Note that this is in lowercase!
        ...
        citations (list[int]):
            ...
            hgvs_c (list[str]):
            Convenience property. All coding DNA HGVS strings (entries where Type
            contains "coding"), assembled as "AccessionVersion:Change".

        hgvs_g (list[str]):
            Convenience property. All genomic HGVS strings (entries where Type
            contains "genomic"), assembled as "AccessionVersion:Change".

        hgvs_p (list[str]):
            Convenience property. All protein-effect HGVS strings (entries where Type
            contains "protein", or coding entries that also carry a ProteinExpression),
            assembled as "ProteinAccessionVersion:ProteinChange".

        rsid (str | None):
            The first dbSNP rsID for this variant as a bare numeric string
            (e.g. "28934872"), or None if no dbSNP cross-reference is present.

        rsids (list[str]):
            All dbSNP rsIDs for this variant as bare numeric strings.
            Handles the several ID formats seen in ClinVar XML and deduplicates them.
        ...
        omim_id (str | None):
            The first OMIM cross-reference ID for this variant
            (e.g. "191092.0006"), or None if not present.

        omim_ids (list[str]):
            All OMIM cross-reference IDs for this variant.

        orphanet_id (str | None):
            The first Orphanet cross-reference ID for this variant, or None.
            Note: as of the current VCV release, Orphanet identifiers appear at
            the condition level rather than in SimpleAllele/XRefList, so this
            will typically be None. Use associated_conditions instead.

        orphanet_ids (list[str]):
            All Orphanet cross-reference IDs for this variant. Subject to the
            same limitation as orphanet_id; typically empty on VCV records.

        medgen_id (str | None):
            The first MedGen cross-reference ID for this variant, or None.
            Note: as of the current VCV release, MedGen identifiers appear at
            the condition level rather than in SimpleAllele/XRefList, so this
            will typically be None. Use associated_conditions instead.

        medgen_ids (list[str]):
            All MedGen cross-reference IDs for this variant. Subject to the
            same limitation as medgen_id; typically empty on VCV records.

        spdi (CanonicalSPDI | None):
            Canonical SPDI notation for this variant
            (e.g. "NC_000001.11:230710047:A:G"), or None if unavailable.
            Only populated for VCV-format records. Call .parse() on the returned
            object to extract chromosome, position, deleted, and inserted alleles
            as a SPDIInfo dataclass.

        record_status (str | None):
            The record status string from ClinVar (e.g. "current").

        version (str | None):
            The version number of this VariationArchive or VariationReport record.
    """
